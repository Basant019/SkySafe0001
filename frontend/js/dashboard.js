 ...
 * Handles: role-based UI, disaster reports (user submit + admin manage), trip management
 */
...
async function fetchUserReports() {
    try {
        const r = await fetch(`${API}/reports/user/${currentUser.id}`);
...
async function fetchUserTrips() {
    try {
        const r = await fetch(`${API}/trips/user/${currentUser.id}`);
        const d = await r.json();
...
async function fetchAllTrips() {
    try {
        const r = await fetch(`${API}/trips/all`);
        const d = await r.json();
        return d.success ? d.trips : [];
    } catch { return []; }
}
